...
run-to-run scatter; the wall-paced chirp comparison self-skips if
the runner cannot hold the sim/wall ratio; desyncs check against a
per-model budget (the 1404 reproduces the real hardware's
marginality by design). Real-vs-run comparison graphs land in
--artifacts.
