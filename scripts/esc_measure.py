...
class Recorder(object):
    ...
    def add(self, rtype, **fields):
        row = {'t': round(time.monotonic() - self.t0, 4), 'type': rtype}
        row.update(fields)
...
class EscMeasure(object):
    ...
    def spin_for(self, duration):
        '''run the RawCommand stream and message pump for duration seconds'''
    ...
    def wait_ready(self):
        ...
        while self.last_esc_status is None:
            self.spin_for(0.2)
            if time.monotonic() - tstart > self.args.ready_timeout:
