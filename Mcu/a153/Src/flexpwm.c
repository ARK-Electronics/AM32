...
void initFlexPWM(void)
{
	...
	modifyReg32(&SYSCON->PWM0SUBCTL, SYSCON_PWM0SUBCTL_CLK0_EN_MASK | SYSCON_PWM0SUBCTL_CLK1_EN_MASK | SYSCON_PWM0SUBCTL_CLK2_EN_MASK,
	...
	for (int submodule = 0; submodule <= 2; submodule++) {
		...
		modifyReg16(&FLEXPWM0->SM[submodule].CTRL, PWM_CTRL_PRSC_MASK | PWM_CTRL_LDMOD_MASK, PWM_CTRL_PRSC(0) | PWM_CTRL_LDMOD(0));

		//Enable complementary channel operation
		//FRCEN must be disabled. Otherwise the motor has hiccups during phase switching
		modifyReg16(&FLEXPWM0->SM[submodule].CTRL2, PWM_CTRL2_INDEP_MASK | PWM_CTRL2_FRCEN_MASK,
		...
		modifyReg16(&FLEXPWM0->SM[submodule].DISMAP[0], PWM_DISMAP_DIS0X_MASK | PWM_DISMAP_DIS0B_MASK | PWM_DISMAP_DIS0A_MASK, 0);
	...
	modifyReg16(&FLEXPWM0->SWCOUT, 0, PWM_SWCOUT_SM0OUT23_MASK | PWM_SWCOUT_SM1OUT23_MASK | PWM_SWCOUT_SM2OUT23_MASK);
