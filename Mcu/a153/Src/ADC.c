...
void calibADC(void)
{
	...
	float gainAdjustment =
		(float)((131072.0) / (131072.0 - (double)gain_cal)); // Gain_CalA = (131072.0 / (131072-(ADC_GCC_GAIN_CAL(ADC->GCC[0]))
...
void initADC(void)
{
	...
	//Enable ADC clock
	modifyReg32(&MRCC0->MRCC_ADC0_CLKDIV, MRCC_MRCC_ADC0_CLKDIV_DIV_MASK | MRCC_MRCC_ADC0_CLKDIV_HALT_MASK,
	...
	modifyReg32(&SENSE_ADC_PORT->PCR[VOLTAGE_SENSE_ADC_PIN], 0xffff, 0);
	modifyReg32(&SENSE_ADC_PORT->PCR[CURRENT_SENSE_ADC_PIN], 0xffff, 0);
	...
	modifyReg32(&ADC0->CMD[0].CMDL, ADC_CMDL_MODE_MASK | ADC_CMDL_CTYPE_MASK | ADC_CMDL_ADCH_MASK,
		    ADC_CMDL_MODE(1) | ADC_CMDL_CTYPE(0) | ADC_CMDL_ADCH(CURRENT_ADC_CHANNEL));
	modifyReg32(&ADC0->CMD[0].CMDH, ADC_CMDH_NEXT_MASK | ADC_CMDH_STS_MASK, ADC_CMDH_NEXT(2) | ADC_CMDH_STS(2));

	//Set CMD2 for measured voltage
	modifyReg32(&ADC0->CMD[1].CMDL, ADC_CMDL_MODE_MASK | ADC_CMDL_CTYPE_MASK | ADC_CMDL_ADCH_MASK,
		    ADC_CMDL_MODE(1) | ADC_CMDL_CTYPE(0) | ADC_CMDL_ADCH(VOLTAGE_ADC_CHANNEL));
	modifyReg32(&ADC0->CMD[1].CMDH, ADC_CMDH_NEXT_MASK | ADC_CMDH_STS_MASK, ADC_CMDH_NEXT(3) | ADC_CMDH_STS(2));
	 ...
	 */
	modifyReg32(&ADC0->CMD[2].CMDL, ADC_CMDL_MODE_MASK | ADC_CMDL_CTYPE_MASK | ADC_CMDL_ADCH_MASK,
		    ADC_CMDL_MODE(1) | ADC_CMDL_CTYPE(0) | ADC_CMDL_ADCH(TEMP_ADC_CHANNEL));
#ifndef USE_ADC_INPUT
	modifyReg32(&ADC0->CMD[2].CMDH, ADC_CMDH_NEXT_MASK | ADC_CMDH_STS_MASK,
	...
	modifyReg32(&ADC0->CMD[3].CMDL, ADC_CMDL_MODE_MASK | ADC_CMDL_CTYPE_MASK | ADC_CMDL_ADCH_MASK,
		    ADC_CMDL_MODE(1) | ADC_CMDL_CTYPE(0) | ADC_CMDL_ADCH(0));
	modifyReg32(&ADC0->CMD[3].CMDH, ADC_CMDH_NEXT_MASK | ADC_CMDH_STS_MASK, ADC_CMDH_NEXT(0) | ADC_CMDH_STS(2));
#endif
...
int16_t computeTemperature(uint16_t raw_temp_val1, uint16_t raw_temp_val2)
{
	...
	float tmp = (float)slopeFactor * ((bandgapCoefficient * ((float)raw_temp_val2 - (float)raw_temp_val1)) /
					  ((float)raw_temp_val2 + (bandgapCoefficient * ((float)raw_temp_val2 - (float)raw_temp_val1)))) -
		    offsetFactor;
